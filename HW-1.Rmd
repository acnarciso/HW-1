...
* What gave you the most trouble: Trying to figure out what combination of commands to use and how to solve an ambiguous problem.
...
flights_airports_region <- left_join(flights_airports_july, states, by="state")
...
# SW_flight_paths_by_state <- southwest_flights %>% 
#   group_by(state, dest) %>% 
#   tally() %>% 
#   rename(num_flight_paths = n)
# SW_flight_paths_by_state
...
# Tallies the total number of flights
flights_july_total <- flights_region_july %>% 
  group_by(region) %>% 
  summarise(total_num_flights = sum(n))
# flights_july_total
# PRODUCES AN OVERLY HIGH NUMBER????

# Tallies the total number of flights by region
flights_region_july_tally <- flights_region_july %>% 
  group_by(region) %>% 
  tally() %>% 
  rename(num_flights_per_region = n)
# flights_region_july_tally

# Finds the proportion of flights per region
flights_region_july_prop <- inner_join(flights_region_july_tally, 
                                       flights_july_total, by="region") %>% 
  mutate(prop = round(num_flights_per_region/total_num_flights, 2)) %>% 
  select(region, prop) %>% 
